# AgntSignals

Telegram bot that publishes Forex trade signals publicly and executes automated trades via an owner-controlled pooled account for invite-only paid subscribers.

Spec: [`docs/blueprint.md`](docs/blueprint.md).

Built on [agnt-gm.ai](https://agnt-gm.ai). The whole bot is built and refined here as pull requests across successive build passes.
